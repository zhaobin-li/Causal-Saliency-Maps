...
df |>
  group_by(id) |>
  summarize(minRTsecs = mean(rt) / 1000) |>
  ggplot(aes(minRTsecs)) +
...
df <- df |>
  rename(img = imgNum) |>
  select(id, img, responseCompare, labelCompare) |>
  glimpse()

df <- df |>
  mutate(
    responseCompare = fct_relevel(factor(responseCompare), ref = "same"),
    labelCompare = fct_relevel(factor(labelCompare), ref = "same")
  ) |>
  glimpse()

df |>
  group_by(labelCompare, responseCompare) |>
  summarise(n())

df |>
  group_by(labelCompare, id, responseCompare) |>
  summarize(n = n()) |>
  group_by(labelCompare) |>
  complete(id, responseCompare, fill = list(n = 0)) |>
  group_by(labelCompare, id) |>
  mutate(p = n / sum(n)) |>
  filter(responseCompare == "opp") |>
  ggplot(aes(labelCompare, p)) +
  geom_boxplot() +
  ylim(0, 1) +
  geom_hline(yintercept = 0.5, linetype = "dashed", color = "red", size = 2) +
  labs(y = 'P(responseCompare == "opp")')
...
df <- df |>
  mutate(
    choice = ifelse(responseCompare == "same", 1, 0),
    label = ifelse(labelCompare == "same", 1, 0),
  ) |>
  glimpse()
...
side |>
  ggplot(aes(as.numeric(meanMainRTsecs))) +
  geom_dotplot()

side |>
  ggplot(aes(as.numeric(totalTimeElapsedmins))) +
  geom_dotplot()
